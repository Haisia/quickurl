@echo off
REM K6 URL 클릭 부하 테스트 실행 스크립트 (Windows)

echo ============================================
echo K6 URL 클릭 부하 테스트 시작
echo ============================================
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    for /f "delims=" %%f in ('dir /b /o-d ".\output\click-test_*.html" 2^>nul') do (
        set "LATEST_HTML=%%f"
        goto :found_html
    )
    ...
    if defined LATEST_HTML (
        ...
        REM JSON 파일도 표시
        for /f "delims=" %%j in ('dir /b /o-d ".\output\click-test_*.json" 2^>nul') do (
