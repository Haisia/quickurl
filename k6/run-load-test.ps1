# K6 부하 테스트 실행 스크립트 (PowerShell)
# 사용법: .\run-load-test.ps1

Write-Host "============================================" -ForegroundColor Cyan
Write-Host "K6 부하 테스트 시작" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    # 생성된 파일 찾기
    $htmlFiles = Get-ChildItem -Path $outputDir -Filter "summary_*.html" | Sort-Object LastWriteTime -Descending
    $jsonFiles = Get-ChildItem -Path $outputDir -Filter "summary_*.json" | Sort-Object LastWriteTime -Descending
    ...
    if ($htmlFiles.Count -gt 0) {
        ...
        if ($jsonFiles.Count -gt 0) {
            try {
                ...
                if ($summary.metrics.url_creation_count) {
                    $urlCount = $summary.metrics.url_creation_count.values.count
                    Write-Host "생성된 URL 수: $urlCount 개" -ForegroundColor Yellow
                }
